...
Whenever a Trigger fires it tells the Engine a target Function to call and the payload to send. The Engine itself handles the routing and sends that message along with a context on to a Worker that contains that Function to can handle the request.
