...
Whenever a Trigger fires it tells the Engine a target Function to call and the payload to send it. The Engine itself handles the routing and sends that message on to a Worker that contains that Function and can handle the request.
