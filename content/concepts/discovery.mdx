...
sequenceDiagram
    ...
    WA->>E: connect + register orders::create
    E->>WA: functionsAvailable [orders::create, enqueue, state::set, ...]

    WB->>E: connect + register payments::charge
    E->>WA: functionsAvailable [orders::create, payments::charge, ...]
    E->>WB: functionsAvailable [orders::create, payments::charge, ...]

    Note over WA,WB: Worker B can now trigger orders::create
    Note over WA,WB: Worker A can now trigger payments::charge

    WB->>E: disconnect
    E->>WA: functionsAvailable [orders::create, enqueue, state::set, ...]
    Note over WA: payments::charge is gone
```
