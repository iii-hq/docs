---
title: 'Function'
description: 'Register any functionality with the iii engine and make it triggerable from anywhere — any language, any worker.'
---

A Function is an async handler identified by a string ID. You write it in any language (TypeScript, Python, Rust), register it with the engine over WebSocket, and it becomes triggerable from **anywhere** in the system — other workers, other languages, other machines. The engine handles routing, serialization, and delivery.
...
There are three ways to trigger a registered Function:

### `trigger()` — trigger and get the result
...
Bind a Function to an event source so it triggers automatically when something happens:
...
A Python worker can trigger a TypeScript function and vice versa. The engine handles serialization and routing transparently:
...
<Cards>
  <Card icon={<Zap />} title="Triggers" href="/docs/concepts/trigger">
    Learn all the ways to trigger a Function.
  </Card>
