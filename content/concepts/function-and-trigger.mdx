...
iii.register_trigger(
    "cron",
    "math::aggregation",
    json!({ "cron": "0 */30 * * * *" }), // every 30 minutes
)?;
```
</Tab>
</Tabs>
...
iii.register_function("orders::webhook", |event| async move {
    ...
    client.post(&url)
        ...
        .send()
        .await
        .map_err(|e| iii_sdk::IIIError::Other(e.to_string()))?;
    Ok(json!({}))
});
...
<Tab value="Rust">
```rust
let t = iii.register_trigger(TriggerConfig {
    trigger_type: "http",
    function_id: "fn::id",
    config: json!({"api_path": "/ping", "http_method": "GET"}),
})?;
t.unregister();
```
</Tab>
