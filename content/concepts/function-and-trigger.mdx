...
iii.registerTrigger({
    type: 'http',
    function_id: 'math::work',
    config: { api_path: '/math/add', http_method: 'POST' },
})
// runs a heavy math function via http
```
</Tab>
<Tab value="Python">
```python
...
iii.register_trigger(
    "http",
    "math::work",
    json!({ "api_path": "/math/add", "http_method": "POST" }),
)
...
iii.registerTrigger({
    ...
    config: { cron: '0 */30 * * * * *' }, // 7-field cron: seconds minutes hours dom month dow year
})
// runs a math aggregation function every 30 minutes
```
</Tab>
<Tab value="Python">
```python
...
iii.register_trigger(
    "cron",
    "math::aggregation",
    json!({ "cron": "0 */30 * * * * *" }), // 7-field cron: seconds minutes hours dom month dow year
)
...
// runs an audit log whenever state updates
```
</Tab>
<Tab value="Python">
```python
...
The triggering worker doesn't know what language the target is written in or where it's running.
