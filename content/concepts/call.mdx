---
title: 'Call'
description: 'Invoke any registered function from anywhere in your application.'
hideCards: true
---

Call runs any function registered with the Engine regardless of where it lives, what language it's written in,
or how it was registered.

Without Call, invoking a function across services means knowing its location, protocol, and interface.
Call removes that burden by routing every invocation through the Engine, which already holds a reference to the function and its interface.
The developer supplies the intent; the Engine handles the rest.

```mermaid
graph LR
    FnRef -.-> fn["function()"]
    FnRef --> fn
    Register["registerFunction(function)"] -.-> FnRef
    subgraph Eng[Engine]
        FnRef[Function Reference]:::solid
    end
    Caller["Caller"] --> Call((Call)) --> FnRef
    classDef solid stroke-dasharray: 0
```

```typescript
import { init } from "iii-sdk";

const { call } = init("ws://localhost:49134");

// Call functions in other services asynchronously without knowing their location, protocol,
// or locally redefining its interface.
const dataRequest = call("data-service::transform", { data });
const computeRequest = call("compute-service::compute", { n });

const [dataResult, computeResult] = await Promise.allSettled([
  dataRequest,
  computeRequest,
]);
```

<Cards>
  <Card icon={<Terminal />} title="Quickstart" href={`/docs/tutorials/quickstart`}>
    Build your first application and see Call and Function Registration in action.
  </Card>
  <Card icon={<LayoutTemplate />} title="Registration" href={`/docs/concepts/registration`}>
    Learn how functions become available to the Engine for Call to invoke.
  </Card>
</Cards>
