...
<ResponseField name="kv_server::update" type="function">
  Update a value by key in the KV store using patch operations.
  ...
  <AccordionGroup>
    <Accordion iconName="settings" title="Parameters">
      ...
      <ResponseField name="ops" type="any[]" required>
        An array of patch operations to apply to the value.
      </ResponseField>
    </Accordion>
    <Accordion title="Returns">
      <ResponseField name="result" type="object">
        The result of the update operation containing the updated value.
      </ResponseField>
    </Accordion>
  </AccordionGroup>
</ResponseField>
...
```typescript
await iii.trigger('kv_server::set', { index: 'default', key: 'user:1', value: { name: 'Alice' } })
await iii.trigger('kv_server::set', { index: 'default', key: 'user:2', value: { name: 'Bob' } })
await iii.trigger('kv_server::set', { index: 'default', key: 'product:1', value: { name: 'Widget' } })

const userKeys = await iii.trigger('kv_server::list_keys_with_prefix', {
  prefix: 'user:',
})
```
