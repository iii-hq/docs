---
title: 'Engine'
description: 'The Engine is primarily responsible for routing requests.'
---

The Engine is primarily responsible for routing requests. When a trigger invokes a Function the Engine itself handles the routing and sends that message to a Worker that contains that Function.

The iii Engine is the central orchestrator of a iii-powered system. It knows where every part of your infrastructure lives and routes requests to those parts.

This architecture is what enables application code to be decoupled from the infrastructure it runs on. iii is completely architecture agnostic and makes no distinction between cloud providers, colocated servers, serverless functions, domains, databases, and existing infrastructure.

```mermaid
graph TB
    User((User)) -->|invokes| Trigger

    subgraph Trigger[Trigger]
        Config[POST /users]
    end
    subgraph Engine[Engine]
        Fn["(target_function, payload)"]
    end
    Trigger -->|"(create_user, payload)"| Engine
    subgraph Worker[Worker]
        Fn2[Function]
        Fn3[Function]
        Fn4[Function]
    end
Engine -->|"(create_user, payload)"| Worker
```

<Callout title="Context" type="info">
  In addition to orchestration the Engine also keeps track of the system's Context. This enables a shared context
  where any Function can access anything that the Engine has access to.
</Callout>
