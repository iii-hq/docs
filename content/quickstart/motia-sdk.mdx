...
config = StepConfig(
    name="hello",
    triggers=[
        ApiTrigger(path="/hello/:name", method="GET")
    ]
)
...
> **Learn More**: For technical details on trigger indexing, TriggerInput normalization, and condition evaluation flow, see [SDK Implementation](/advanced/sdk-implementation#multi-trigger-architecture).
...
<Columns cols={2}>
  <Card icon={<Code />} title="Base SDK" href="/quickstart/iii-sdk">
    Learn about the low-level iii SDK
  </Card>
  <Card icon={<Activity />} title="Streams" href="/core-modules-list/module-stream">
