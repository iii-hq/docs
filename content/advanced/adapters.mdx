...
Each iii module that needs persistence or distribution uses an **adapter** — a pluggable backend that implements a fixed interface. Swap adapters in `config.yaml` without touching application code.
...
<Callout type="info" title="Environment variables">
  Use `${VAR:default}` syntax in `config.yaml` to switch adapters per environment without changing the file:
