...
> **See Examples**: For practical examples of multi-trigger steps and conditions in action, see [Motia SDK - Multiple Triggers](/docs/tutorials/motia-sdk#multiple-triggers) and [Trigger Conditions](/docs/tutorials/motia-sdk#trigger-conditions).
...
Streams are accessed via the `stream::` namespace functions invoked over the bridge.

| Operation     | SDK Function                 | Bridge Target        | Description                    |
| ------------- | ---------------------------- | -------------------- | ------------------------------ |
| **Get**       | `stream.get(group, id)`      | `stream::get`        | Retrieves an item from a group |
| **Set**       | `stream.set(group, id, val)` | `stream::set`        | Saves an item to a group       |
| **Delete**    | `stream.delete(group, id)`   | `stream::delete`     | Removes an item                |
| **Get Group** | `stream.list(group)`         | `stream::list`       | Retrieves all items in a group |
...
<Columns cols={2}>
  <Card icon={<Code />} title="Motia Framework" href="/docs/tutorials/motia-framework">
