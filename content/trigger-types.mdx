...
**Conditions**: Optional. Add `_condition_path` to config with a function ID. The engine invokes it before the handler; if it returns `false`, the handler is skipped. See [Trigger Conditions](#trigger-conditions).
...
2. Add `condition_function_id` to the trigger config (or `_condition_path` for http/queue triggers).
...
const conditionFn = iii.registerFunction(
  ...
  async (event) => event.event_type === 'state:updated' && event.key === 'profile',
)
...
| cron | `condition_function_id` |
| stream, stream:join, stream:leave | `condition_function_id` |
