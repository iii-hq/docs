...
graph LR
    ...
    HelloHandler -->|"queue::enqueue: greet.process"| ProcessHandler
...
- `iii.call('queue::enqueue', { topic, data })` publishes an event on the named topic. Any worker with a `queue` trigger on that topic will receive it.
