...
```mermaid
graph LR
    Client -->|"POST /hello"| HelloHandler
    HelloHandler -->|"queue::emit: greet.process"| ProcessHandler
...
```typescript
// Start the worker — connect to engine and keep alive
await iii.connect()
```
...
- `iii.call('queue::emit', { topic, data })` publishes an event on the named topic. Any worker with a `queue` trigger on that topic will receive it.
- `iii.call('state::set', { scope, key, data })` persists data to the engine's key-value store, addressed by `scope` + `key`.
