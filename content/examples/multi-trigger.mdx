...
graph LR
    ...
    Handler -->|"queue::emit: order.processed"| Downstream
```
...
iii.registerFunction(
  ...
  async (input) => {
    ...
    const pendingOrders = await iii.call<{ id: string; amount: number }[]>('state::list', {
...
async def orders_handle(data, ctx) -> dict | None:
    ...
    pending = await iii.call("state::list", {"scope": "pending-orders"}) or []
...
iii.register_function("orders.handle", |input| async move {
    ...
    let pending = iii.call("state::list", json!({ "scope": "pending-orders" })).await?;
...
- Queue trigger input is the raw event payload you published with `queue::emit`.
