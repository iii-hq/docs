...
The `cron` trigger type fires a registered function on a schedule. The function receives `null` / `None` input — the invocation itself is the signal. Work is typically fanned out by calling `queue::emit` to downstream queue handlers.

```mermaid
graph LR
    Cron -->|"every minute"| CronFn
    CronFn -->|"queue::emit: job.tick"| JobHandler
...
- Keep cron handlers lightweight. Use `queue::emit` to fan out work to queue handlers that can run in parallel.
