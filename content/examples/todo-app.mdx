...
iii.registerFunction(
  ...
  async (req: ApiRequest<{ description: string; dueDate?: string }>) => {
    ...
    const todo = await iii.call('stream::set', {
      stream_name: 'todo',
      group_id: 'inbox',
      item_id: todoId,
      data: {
...
async def _create_todo(req: ApiRequest, ctx) -> ApiResponse:
    ...
    todo = await iii.call("stream::set", {
        "stream_name": "todo",
        "group_id": "inbox",
        "item_id": todo_id,
        "data": {
...
```typescript
iii.registerFunction(
  { id: 'api.put.todo/:id', description: 'Update a todo' },
  async (req: ApiRequest) => {
    const ctx = getContext()
    const todoId = req.path_params?.id

    const existing = todoId
      ? await iii.call<Todo | null>('stream::get', {
          stream_name: 'todo',
          group_id: 'inbox',
          item_id: todoId,
        })
    ...
    const updated = await iii.call('stream::set', {
      stream_name: 'todo',
      group_id: 'inbox',
      item_id: todoId,
...
async def _update_todo(req: ApiRequest, ctx) -> ApiResponse:
    ...
    existing = await iii.call("stream::get", {
    ...
    updated = await iii.call("stream::set", {
...
iii.register_function("api.delete.todo", |input| async move {
    ...
    iii.call_void("stream::delete", json!({
        "stream_name": "todo",
        "group_id": "inbox",
        "item_id": todo_id,
    }))?;
