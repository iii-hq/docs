...
| Publish event | `iii.trigger('queue::enqueue', { topic, data })` | `await iii.trigger("queue::enqueue", {...})` | `iii.trigger_void("queue::enqueue", json!({...}))?` |
| State write | `iii.trigger('state::set', { scope, key, data })` | `await iii.trigger("state::set", {...})` | `iii.trigger_void("state::set", json!({...}))?` |
