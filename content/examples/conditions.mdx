---
title: 'Conditions'
description: 'Pre-handler functions the engine evaluates before executing a trigger — if the condition returns false, the handler is skipped.'
hideCards: true
---

Conditions are regular iii functions that the engine calls **before** a trigger's handler runs. Attach a condition to any trigger by adding the condition function's ID to the trigger config. If the condition returns `false`, the handler is skipped entirely.

```mermaid
graph LR
    Trigger -->|event data| Condition{{"condition fn"}}
    Condition -->|"true"| Handler[handler fn]
    Condition -->|"false"| Skip[skip handler]
```
...
The condition function receives the full API request. If it returns `false`, the engine responds with `422 Unprocessable Entity` and the handler never runs.
...
iii.register_function("orders.premium", lambda data: orders_premium(data, get_context()))
...
The condition function receives the queue message data. If it returns `false`, the message is silently skipped — no error is raised.
...
- When a condition returns `false`:
  - **HTTP** — the engine responds with `422 Unprocessable Entity`; the handler never runs.
  - **Queue / Cron** — the event is silently skipped; no error is surfaced.
  - **State / Stream** — the event is silently skipped.
- If a condition function errors, the engine logs the error and skips the handler (HTTP returns `500 Internal Server Error`).
