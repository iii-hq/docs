...
State in iii is a distributed key-value store addressed by `scope` (group) + `key` (item ID). Any worker can read and write state by calling `state::get`, `state::set`, `state::delete`, and `state::list` through the engine.
...
graph LR
    ...
    AuditHandler -->|"queue::emit: notification"| Notify
```
...
async def update_user_status(req: ApiRequest, ctx) -> ApiResponse:
    ...
    await iii.call("state::set", {
        "scope": "users",
        "key": user_id,
        "data": {"status": new_status, "updatedAt": datetime.now(timezone.utc).isoformat()},
    })
...
iii.registerFunction(
  ...
  async (req: ApiRequest) => {
    ...
    const user = await iii.call<{ status: string } | null>('state::get', {
      scope: 'users',
      key: userId,
    })
...
async def get_user_status(req: ApiRequest, ctx) -> ApiResponse:
    ...
    user = await iii.call("state::get", {"scope": "users", "key": user_id}) if user_id else None
...
iii.register_function("users.get_status", |input| async move {
    ...
    let user = iii.call("state::get", json!({
        "scope": "users",
        "key": user_id,
    })).await?;
...
iii.registerFunction(
  ...
  async () => {
    ...
    const orders = await iii.call<{ id: string; shipDate: string; complete: boolean; status: string }[]>(
      'state::list',
      { scope: 'orders' },
    )
...
async def orders_audit(_data, ctx) -> None:
    orders = await iii.call("state::list", {"scope": "orders"}) or []
...
iii.register_function("cron.orders_audit", |_input| async move {
    ...
    let orders_val = iii.call("state::list", json!({ "scope": "orders" })).await?;
...
|---|---|---|---|
| Write | `iii.call('state::set', { scope, key, data })` | `await iii.call("state::set", {...})` | `iii.call_void("state::set", json!({...}))` |
| Read one | `iii.call('state::get', { scope, key })` | `await iii.call("state::get", {...})` | `iii.call("state::get", json!({...})).await` |
| Read all in scope | `iii.call('state::list', { scope })` | `await iii.call("state::list", {...})` | `iii.call("state::list", json!({...})).await` |
| Delete | `iii.call('state::delete', { scope, key })` | `await iii.call("state::delete", {...})` | `iii.call_void("state::delete", json!({...}))` |
...
- State is backed by the engine's `kv_server` module internally. It is separate from named streams — streams have real-time WebSocket push; state does not.
