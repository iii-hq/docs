...
<AccordionGroup>
  ...
  <Accordion title="Function Organization">
    ...
    ```typescript
    // User functions
    iii.registerFunction({ function_path: 'users::create' }, createHandler)
    iii.registerFunction({ function_path: 'users::update' }, updateHandler)
    iii.registerFunction({ function_path: 'users::delete' }, deleteHandler)

    // Auth functions
    iii.registerFunction({ function_path: 'auth::login' }, loginHandler)
    iii.registerFunction({ function_path: 'auth::logout' }, logoutHandler)
    ```
