...
<AccordionGroup>
  ...
  <Accordion title="Service Organization">
    ...
    bridge.registerFunction({ functionPath: 'users.update', ... });
    bridge.registerFunction({ functionPath: 'users.delete', ... });

    // Auth service
    bridge.registerService({ id: 'auth' });
    bridge.registerFunction({ functionPath: 'auth.login', ... });
    bridge.registerFunction({ functionPath: 'auth.logout', ... });
    ```
  </Accordion>
</AccordionGroup>
...
<Columns cols={2}>
  <Card icon="Code" title="SDK Quick Start" href="/docs/quickstart/iii-sdk">
