...
export function Mermaid({ chart }: { chart: string }) {
    ...
    async function renderDiagram() {
      ...
      if (svgElement) {
        ...
        const strokeColor = style
          .getPropertyValue("--color-mermaid-stroke")
          .trim();
        const textColor = style.getPropertyValue("--color-mermaid-text").trim();
        const clusterFill = style
          .getPropertyValue("--color-mermaid-cluster-fill")
          .trim();
        ...
        svgElement
          .querySelectorAll("polygon, circle, ellipse")
          .forEach((el) => {
            const svgEl = el as SVGElement;
        ...
        svgElement
          .querySelectorAll("text, tspan, .nodeLabel, .edgeLabel")
          .forEach((el) => {
            const svgEl = el as SVGElement;
        ...
        svgElement
          .querySelectorAll("marker path, marker polygon")
          .forEach((el) => {
            const svgEl = el as SVGElement;
  ...
  return (
    <div ref={containerRef} className="mermaid my-6 flex justify-center" />
  );
}
